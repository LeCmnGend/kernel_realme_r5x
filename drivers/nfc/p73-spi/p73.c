...
static int p61_probe(struct spi_device *spi)
{
	...
	if (!p61_dev->kbuf) {
		pr_err("failed to allocate memory for nqx_dev->kbuf\n");
		ret = -ENOMEM;
		goto err_exit;
	}
...
static int __init p61_dev_init(void)
{
    ...
    if((strncmp(nfc_hardware_info,"S86125FA1",strlen("S86125FA1")) == 0)||
      ...
      (strncmp(nfc_hardware_info,"S86125PA1",strlen("S86125PA1")) == 0)||
      (strncmp(nfc_hardware_info,"S86127CA1",strlen("S86127CA1")) == 0)||
      (strncmp(nfc_hardware_info,"S86127FA1",strlen("S86127FA1")) == 0)||
      (strncmp(nfc_hardware_info,"S86125GA1",strlen("S86125GA1")) == 0)||
      (strncmp(nfc_hardware_info,"S86125JA1",strlen("S86125JA1")) == 0)||
      (strncmp(nfc_hardware_info,"S86129IA1",strlen("S86129IA1")) == 0)||
      (strncmp(nfc_hardware_info,"S86129JA1",strlen("S86129JA1")) == 0)||
      (strncmp(nfc_hardware_info,"S86129KA1",strlen("S86129KA1")) == 0)){
	    P61_DBG_MSG("Entry : %s. BoardID:%s\n", __FUNCTION__,nfc_hardware_info);
	    return spi_register_driver(&p61_driver);
    }
    else{
